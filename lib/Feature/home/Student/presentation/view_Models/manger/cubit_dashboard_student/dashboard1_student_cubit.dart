...
class Dashboard1StudentCubit extends Cubit<Dashboard1StudentState> {
  ...
  Future<void> getStudentDashboardData() async {
    ...
    result.fold(
      (failure) => emit(Dashboard1StudentFailure(errMessage: failure.errorMessage)),
      
     
      (successModel) {  if (!isClosed) { emit(Dashboard1StudentSuccess(studentDashboardModel: successModel));}}
    );
  }
}